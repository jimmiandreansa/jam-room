...
type RoomYouTubeSearchProps = {
  ...
  disabled?: boolean;
  /** Suggestions shown when the input is empty (reuses room recommendations). */
  suggestions?: YoutubeSearchHit[];
  suggestionsLoading?: boolean;
};

const SUGGESTIONS_LIMIT = 8;

export function RoomYouTubeSearch({
  onPick,
  disabled,
  suggestions = [],
  suggestionsLoading = false,
}: RoomYouTubeSearchProps) {
  ...
  const suggestionItems = suggestions.slice(0, SUGGESTIONS_LIMIT);
  const showResults =
    open &&
      ...
      <div className="relative z-50">
        ...
        {showPanel && (
          <div
            role="listbox"
            aria-label={
              showSuggestions ? "Rekomendasi lagu" : "Hasil pencarian YouTube"
            }
          ...
          >
            {showSuggestions ? (
              <>
                <p className="px-4 pb-1 pt-2 text-xs font-medium uppercase tracking-wide text-jam-muted">
                  Rekomendasi untukmu
                </p>
                {suggestionsLoading && suggestionItems.length === 0 && (
                  <p className="px-4 py-3 text-sm text-jam-muted">
                    Memuat rekomendasi…
                  </p>
                )}
                {suggestionItems.map((hit) => (
                  <button
                    key={hit.videoId}
                    type="button"
                    role="option"
